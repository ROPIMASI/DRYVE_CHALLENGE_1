 ...
 */
@Service
public class ModelsYearsRegistryService {
	
	@Autowired
	ModelYearRepository modelYearRepo;
	...
	public long findKBBId(ModelEntity model, short year) {
		...
		myEntity = modelYearRepo.findByModelIdAndYear(model.getId(), year);
		
		/* FURTHER: Errors/Exceptions handling. */
		return myEntity.getKbbId();
